        ...
        <div class="detail-gallery">
            ...
            <div class="product-image-slider">
                <figure class="border-radius-10">
                    <img src="{{ $product->image_url ?? asset('logo/Rasa Connect - Logo 2_Maroon 1.png') }}" alt="{{ $product->display_name }}" onerror="this.onerror=null;this.src='{{ asset('logo/Rasa Connect - Logo 2_Maroon 1.png') }}';" @if(!$product->image) style="object-fit: contain; padding: 2rem;" @endif />
                </figure>
                @if($product->images)
                    @foreach(json_decode($product->images) as $img)
                        <figure class="border-radius-10">
                            <img src="{{ asset('storage/' . $img) }}" alt="{{ $product->display_name }}" onerror="this.onerror=null;this.src='{{ asset('logo/Rasa Connect - Logo 2_Maroon 1.png') }}';" />
                        </figure>
                    @endforeach
                @endif
            </div>
            ...
            <div class="slider-nav-thumbnails">
                <div><img src="{{ $product->image_url ?? asset('logo/Rasa Connect - Logo 2_Maroon 1.png') }}" alt="{{ $product->display_name }}" onerror="this.onerror=null;this.src='{{ asset('logo/Rasa Connect - Logo 2_Maroon 1.png') }}';" @if(!$product->image) style="object-fit: contain; padding: 1rem;" @endif /></div>
                @if($product->images)
                    @foreach(json_decode($product->images) as $img)
                        <div><img src="{{ asset('storage/' . $img) }}" alt="{{ $product->display_name }}" onerror="this.onerror=null;this.src='{{ asset('logo/Rasa Connect - Logo 2_Maroon 1.png') }}';" /></div>
                    @endforeach
                @endif
            </div>
        </div>
