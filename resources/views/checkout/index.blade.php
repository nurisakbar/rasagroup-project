        ...
        <div class="row">
                        ...
                        <div class="payment_accordion">
                            ...
                            @if(config('services.active_payment_gateway') === 'faspay')
                                ...
                                <div class="payment-option mb-10 payment-method-card" id="card-faspay-va-group">
                                    ...
                                    <div class="payment-content pl-20 mt-10" id="va-options-container" style="display: none;">
            ...
            <div class="col-lg-5">
                <div class="cart-totals checkout-order-summary rg-checkout-summary mb-50">
                                ...
                                <tbody>
                                    @foreach($carts as $cart)
                                        @php
                                            ...
                                            $retailUnit = $baseRetailUnit * $multiplier;
                                            ...
                                            <td class="rg-checkout-item-info">
                                                ...
                                                <div class="rg-checkout-item-meta">
                                                    <span class="rg-checkout-item-qty">
                                                        ...
                                                        @if($cart->showsLargeUnitInCart())
                                                            <span class="text-muted font-xs">({{ number_format($cart->quantity) }} {{ $cart->product->unit }})</span>
                                                        @endif
                                                        ×
                                                    </span>
                                                ...
                                                @if($showRetailStrike)
                                                    <div class="d-flex flex-column align-items-end">
                                                        <span class="rg-checkout-item-unit-retail text-muted text-decoration-line-through" style="font-size: 0.85em;">
                                                            Rp {{ number_format($baseRetailUnit * $cart->quantity, 0, ',', '.') }}
                                                        </span>
                                                        <strong class="rg-checkout-item-price-value text-brand">
                                                            Rp {{ number_format($baseUnitPrice * $cart->quantity, 0, ',', '.') }}
                                                        </strong>
                                                        <span class="badge bg-danger mt-1">-{{ $discountPercentage }}%</span>
                                                    </div>
                                                @else
                                                    <strong class="rg-checkout-item-price-value text-brand">Rp {{ number_format($baseUnitPrice * $cart->quantity, 0, ',', '.') }}</strong>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                    ...
                    <!-- Notes -->
                    <div class="mb-20">
                        <h6 class="mb-10"><i class="fi-rs-comment mr-5 text-muted"></i>Catatan Tambahan</h6>
                        <div class="form-group mb-0">
                            <textarea name="notes" rows="2" class="form-control" placeholder="Catatan pesanan (Opsional)">{{ old('notes') }}</textarea>
                        </div>
                    </div>
...
<script>
    ...
    function updateShipping() {
        ...
        if (data.error) {
                ...
                if (data.is_shipping_discounted) {
                    ...
                    $('#shippingCostDisplay').html(`${data.shipping_cost_formatted}`);
                }
    ...
    function selectPayment(method) {
        if (method === 'faspay_va_group') {
             ...
             // Check if a VA is already selected, if not, select the first one
             var currentVAMethod = $('input[name="payment_method"]:checked').val();
             if (!currentVAMethod || !currentVAMethod.startsWith('faspay_') || currentVAMethod === 'faspay_qris') {
                 var firstVA = $('input[name="payment_method"][value^="faspay_"]:not([value="faspay_qris"])').first().val();
                 if (firstVA) {
                     $('input[name="payment_method"][value="' + firstVA + '"]').prop('checked', true);
                     var firstVAId = firstVA.replace('faspay_', 'card-faspay-');
                     $('#' + firstVAId).addClass('active');
                     
                     // Update fee
                     if (paymentFees[firstVA]) {
                         currentPaymentFee = parseFloat(paymentFees[firstVA]);
                     } else {
                         currentPaymentFee = 0;
                     }
                 ...
                 $('#' + currentVAMethod.replace('faspay_', 'card-faspay-')).addClass('active');
             }
             return;
        }
