...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $foreignKeys = DB::select("SELECT CONSTRAINT_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
            WHERE TABLE_NAME = 'addresses' AND TABLE_SCHEMA = DATABASE() AND CONSTRAINT_NAME LIKE 'addresses_%_foreign'");
        $fkNames = array_map(fn($fk) => $fk->CONSTRAINT_NAME, $foreignKeys);

        Schema::table('addresses', function (Blueprint $table) use ($fkNames) {
            if (in_array('addresses_province_id_foreign', $fkNames)) { $table->dropForeign(['province_id']); }
            if (in_array('addresses_regency_id_foreign', $fkNames)) { $table->dropForeign(['regency_id']); }
        ...
        DB::table('addresses')->whereNotExists(function($q){
            $q->select(DB::raw(1))->from('raja_ongkir_provinces')->whereRaw('raja_ongkir_provinces.id = addresses.province_id');
        })->update(['province_id' => null]);

        DB::table('addresses')->whereNotExists(function($q){
            $q->select(DB::raw(1))->from('raja_ongkir_cities')->whereRaw('raja_ongkir_cities.id = addresses.regency_id');
        })->update(['regency_id' => null]);

        DB::table('addresses')->whereNotExists(function($q){
            $q->select(DB::raw(1))->from('raja_ongkir_districts')->whereRaw('raja_ongkir_districts.id = addresses.district_id');
    ...
    public function down(): void
    {
        ...
        Schema::table('addresses', function (Blueprint $table) {
            ...
            $table->string('regency_id')->change();
            $table->string('district_id')->change();

            // Restore original foreign keys if they exist
