...
class OrderController extends Controller
{
    ...
    public function cart()
    {
        ...
        return view('buyer.distributor.orders.cart', compact('carts', 'subtotal', 'totalItems', 'potentialPoints', 'discountPercent', 'discountAmount'));
    }
    ...
    public function checkout()
    {
        ...
        if ($defaultAddress && $defaultExpedition && $defaultService) {
            ...
            if (strtolower($defaultExpedition->code) === 'jne') {
                $shippingCost = $shippingCost * 0.8; // 20% discount
            }
        }
    ...
    public function calculateShipping(Request $request)
    {
        ...
        if ($expedition && strtolower($expedition->code) === 'jne') {
            $shippingCost = $shippingCost * 0.8; // 20% discount
        }
        ...
        return response()->json([
            ...
            'has_shipping_discount' => $shippingCost < $originalShippingCost,
            'subtotal' => $subtotal,
    ...
    public function getExpeditionServices(Request $request)
    {
        ...
        foreach ($expedition->services as $service) {
            ...
            if (strtolower($expedition->code) === 'jne') {
    ...
    public function changePaymentMethod(Request $request, Order $order)
    {
        ...
        try {
            ...
            return redirect()->route('distributor.orders.success', $order)->with('success', 'Metode pembayaran berhasil diubah.');
        } catch (\Exception $e) {
            \DB::rollBack();
    ...
    public function history(Request $request)
    {
        ...
        // Calculate Monthly Target Progress from TargetBelanja
        $currentMonthYear = now()->format('Y-m');
        $targetRecord = \App\Models\TargetBelanja::where('distributor_id', $user->id)
            ->where('bulan_tahun', $currentMonthYear)
            ->first();
            
        $monthlyTarget = $targetRecord ? $targetRecord->jumlah_target : 0;
    ...
    public function confirmReceipt(\App\Models\Order $order)
    {
        ...
        if (!in_array($order->order_status, ['shipped', 'delivered']) && empty($order->shipped_at)) {
            return redirect()->route('distributor.orders.show', $order)->with('error', 'Pesanan belum dapat dikonfirmasi karena belum dikirim/diserahkan.');
        }
        ...
        return redirect()->route('distributor.orders.show', $order)->with('success', 'Terima kasih! Pesanan telah dikonfirmasi selesai dan diterima dengan baik.');
    }
